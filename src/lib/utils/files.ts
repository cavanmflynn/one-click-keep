...
const abs = (path: string): string => {
  return isAbsolute(path) ? path : join(dataPath, path);
};
...
export const write = async (filePath: string, content: string) => {
  return outputFile(abs(filePath), content);
};
...
export const read = async (
  ...
  return (await readFile(abs(filePath))).toString(encoding);
};
...
export const exists = async (filePath: string): Promise<boolean> => {
  return pathExists(abs(filePath));
};
