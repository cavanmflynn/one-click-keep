...
class BitcoindService {
  createClient(node: BitcoinNode) {
    return new BitcoinCore({
      host: '127.0.0.1',
      port: `${node.ports.rpc}`,
      username: BITCOIN_CREDENTIALS.user,
      password: BITCOIN_CREDENTIALS.pass,
      logger: this.log(),
    });
  }
  ...
  async connectPeers(node: BitcoinNode) {
    ...
    for (const peer of node.peers) {
      try {
        ...
        logger.debug(
          `Failed to add peer '${peer}' to bitcoind node ${node.name}`,
          error,
        );
      }
    }
...
export const bitcoindService = new BitcoindService();
